...
if __name__ == "__main__":
    ...
    response = table.get_item(Key=dict(name="toml-lang", owner="github"))
    rid = response["Item"]["id"]
    table = dynamodb.Table("Issues")
    response = table.scan(FilterExpression=Key("repo_id").eq(rid),
                          ProjectionExpression="title, reporter")
